...
interface ContactData {
  hero: {
    titre: string;
    baseline: string;
    video: { url: string; alternativeText: string } | null;
  };
  intro: { titre: string; texte: string };
  closing: { titre: string; texte: string; email: string };
}
...
export default function ContactClient() {
  ...
  const handleSubmit = async (e: React.FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      setSubmitted(true);
      setFormData({ name: '', email: '', organization: '', situation: '', message: '' });
  ...
  if (!contactData) return <div className="loading">Chargement...</div>;
            ...
            {submitted ? (
              ...
              <form onSubmit={handleSubmit} className="contact-form">
                ...
                <div className="contact-form-group">
                  ...
                  <input type="email" id="email" name="email" value={formData.email} onChange={handleChange} required placeholder="jean@exemple.fr" />
                </div>

                <div className="contact-form-group">
                  <label htmlFor="organization">Entreprise/Organisation</label>
                  <input type="text" id="organization" name="organization" value={formData.organization} onChange={handleChange} placeholder="Votre entreprise" />
                </div>
                ...
                <div className="contact-form-group">
                  ...
                  <textarea id="message" name="message" value={formData.message} onChange={handleChange} required placeholder="Décrivez votre projet et ce que vous envisagez..." rows={6} />
                </div>
